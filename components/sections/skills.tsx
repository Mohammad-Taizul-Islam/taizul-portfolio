...
export function SkillsSection({ t }: { t: typeof translations['en'] }) {
  ...
  return (
    <section id="skills" className="min-h-screen py-12 sm:py-16 md:py-20 flex items-center overflow-hidden">
      <div className="container mx-auto px-4">
        <motion.div
          ref={ref}
        ...
        >
          <motion.h2
            ...
            initial={isLargeScreen ? { opacity: 0, y: -50 } : { opacity: 1, y: 0 }}
            animate={isLargeScreen && isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.5, delay: 0.2 }}
          >
            {t.skills}
          ...
          <div className="relative overflow-hidden">
            <motion.div
              ref={containerRef}
              className={`flex gap-6 py-4 ${isLargeScreen ? '' : 'flex-wrap justify-center'}`}
            ...
            >
              {(isLargeScreen ? [...skills, ...skills] : skills).map((skill, index) => (
                <motion.div
                  key={index}
                  className={`flex-shrink-0 ${isLargeScreen ? '' : 'w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 p-2'}`}
                  initial={isLargeScreen ? { opacity: 0, scale: 0.5 } : { opacity: 1, scale: 1 }}
                  animate={isLargeScreen && isInView ? { opacity: 1, scale: 1 } : {}}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                >
                  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6 flex flex-col items-center justify-center w-full h-32 sm:h-40">
                    <skill.icon size={isLargeScreen ? 60 : 40} color={skill.color} />
                    <span className="mt-2 sm:mt-4 text-xs sm:text-sm font-medium text-center">{skill.name}</span>
                  </div>
                </motion.div>
              ))}
            </motion.div>
