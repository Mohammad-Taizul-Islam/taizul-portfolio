...
export function TestimonialsSection({ t }: { t: typeof translations['en'] }) {
  ...
  return (
    <section id="testimonials" className="min-h-screen py-12 sm:py-16 md:py-20 flex items-center">
      <div className="container mx-auto px-4">
        <motion.div
          ref={ref}
          ...
          <div className="relative overflow-hidden">
            ...
            >
              {(isLargeScreen ? [...testimonials, ...testimonials] : testimonials).map((testimonial, index) => (
                <motion.div
                  key={index}
                  className={`${isLargeScreen ? 'w-[400px] shrink-0' : 'w-full mb-6'}`}
                ...
                >
                  <Card>
                    ...
                    <CardContent className="relative">
                      <Quote className="absolute top-0 left-0 w-8 h-8 text-primary/20" />
                      <p className="pl-10 text-sm sm:text-base text-muted-foreground">{testimonial.content}</p>
                    </CardContent>
                  </Card>
                </motion.div>
              ))}
